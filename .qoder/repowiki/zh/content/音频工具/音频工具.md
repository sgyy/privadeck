...
**更新** 本版本重点反映了音量调节工具的重大功能增强，包括波形可视化、双单位支持（百分比和分贝）、实时预览和剪辑检测等功能。同时，音频格式转换工具新增了M4A和Opus格式支持，增强了质量控制和批量处理能力。
...
- 音频格式转换（AudioConvert）
  - 功能：在 MP3、WAV、OGG、AAC、FLAC、M4A、Opus 之间互转，支持批量处理和ZIP打包下载。
  - 关键点：针对每种格式设定最优编码参数与 MIME 类型，新增M4A（AAC容器）和Opus（Ogg容器）支持。
- 音频提取（AudioExtract）
  - 功能：从视频文件中抽取音频轨，输出为 MP3、WAV、AAC、OGG、FLAC、M4A、Opus。
  - 关键点：默认禁用视频轨（-vn），仅提取音频，支持7种输出格式。
- 音量调节（AudioVolume）
  - 功能：实时预览与应用音量增益，支持 50%-300% 的调节范围，新增波形可视化和双单位支持。
  - 关键点：使用 volume 音频滤镜进行幅度缩放，集成 WaveformCanvas 实时波形显示。

**更新** 音量调节工具现在集成了完整的波形可视化系统，支持百分比和分贝两种单位模式，并提供实时预览功能。音频格式转换工具新增了M4A和Opus格式支持，增强了质量控制参数和批量处理能力。
...
### 音频格式转换（AudioConvert）
- 技术要点
  - **更新** 新增格式支持：M4A（AAC容器，MP4/ISO封装，使用ipod复用器）和Opus（Ogg容器，libopus编码器）。
  - 针对每种目标格式设置编码器与质量参数：
    - MP3：libmp3lame，中等质量
    - WAV：PCM 16-bit 无损
    - OGG：libvorbis，中等质量
    - AAC：AAC，192kbps
    - FLAC：无损压缩
    - **新增** M4A：AAC编码，MP4容器，使用ipod复用器避免与原始ADTS .aac混淆
    - **新增** Opus：libopus编码器，Ogg容器封装
  - **更新** 质量控制增强：Opus格式比特率上限256kbps，标准化比特率范围64-320kbps。
  - **更新** 批量处理能力：支持多文件队列处理，单文件处理状态管理，批量ZIP打包下载。
  - MIME 类型与输出扩展名一一对应。
- 数据结构与复杂度
  - 输入为 File；输出为 Blob；复杂度近似 O(n)。
- 性能与质量权衡
  - 有损格式（MP3/OGG/AAC/M4A/Opus）体积小、兼容性好；无损格式（WAV/FLAC）质量高但体积大。
  - **新增** Opus专为VoIP和流媒体优化，适合语音通信和网络广播场景。

```mermaid
flowchart TD
A_START(["开始"]) --> A_LOAD["加载 FFmpeg 单例"]
A_LOAD --> A_MOUNT["挂载输入文件"]
A_MOUNT --> A_BUILD["根据目标格式构建参数<br/>含M4A和Opus支持"]
...
- 技术要点
  - 从视频中抽取音频轨，禁用视频轨（-vn）。
  - **更新** 输出格式扩展：支持 MP3、WAV、AAC、OGG、FLAC、M4A、Opus 七种格式。
  - **更新** 质量控制：针对每种格式设置最优编码参数与比特率/质量参数。
  - **更新** 批量处理：支持多文件队列处理，批量ZIP打包下载。
- 数据结构与复杂度
  - 输入为 File（视频）；输出为 Blob；复杂度近似 O(n)。
- 使用场景
  - 音乐翻录、播客素材提取、配音素材分离、视频内容音频导出等。

```mermaid
flowchart TD
E_START(["开始"]) --> E_LOAD["加载 FFmpeg 单例"]
E_LOAD --> E_MOUNT["挂载输入文件"]
E_MOUNT --> E_ARGS["构建参数禁用视频轨 + 目标音频编码<br/>支持M4A和Opus输出"]
...
- 外部依赖
  ...
  - **新增** fflate（@dnd-kit/core）：用于批量文件ZIP打包下载功能。
- 运行时依赖
...
- 音频格式转换
  - 适用：设备兼容性（MP3/M4A）、归档（FLAC）、网页音频（OGG/Opus）、移动设备（M4A）。
  - **更新** 新格式选择：M4A适合iOS设备和iTunes，Opus适合VoIP和流媒体。
  - 最佳实践：根据用途选择有损/无损；注意目标格式的浏览器支持情况；使用批量功能提高效率。
  - **新增** 批量处理：拖入多个文件，统一设置参数，一键ZIP打包下载。
- 音频提取
  - 适用：从视频中提取音乐、对话、旁白，支持7种输出格式。
  - 最佳实践：优先选择与视频匹配的音频编码（如 AAC/M4A），兼顾体积与兼容性；使用批量功能处理多个视频文件。
- 音量调节
  - 适用：统一音量、提升弱录音。
...
- 协作方式
  ...
  - **新增** 批量处理：音频工具与视频工具共享批量处理架构，支持ZIP打包下载。
...
- **新增** 批量处理问题
  - 现象：批量转换卡住或ZIP下载失败。
  - 处理：检查浏览器内存使用情况；减少同时处理的文件数量；确保网络连接稳定。
- **新增** 新格式兼容性
  - 现象：M4A或Opus格式处理失败。
  - 处理：确认浏览器支持相应的编解码器；检查文件格式是否正确；尝试其他格式。
...
**更新** 本次更新重点增强了音量调节工具的功能完整性，通过集成波形可视化、双单位支持、实时预览和剪辑检测等特性，显著提升了用户体验和专业性。新增的共享组件架构也提高了代码复用性和开发效率。音频格式转换工具新增的M4A和Opus支持进一步扩大了格式覆盖范围，增强了质量控制参数和批量处理能力，使工具能够满足更广泛的专业音频处理需求。这些改进使得音量调节工具不仅适合普通用户，也能满足专业音频工程师的需求。