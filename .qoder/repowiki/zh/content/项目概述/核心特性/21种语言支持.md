...
- [common.json (西班牙语)](file://messages/es/common.json)
- [tools-video.json (西班牙语)](file://messages/es/tools-video.json)
- [tools-developer.json (西班牙语)](file://messages/es/tools-developer.json)
- [tools-image.json (西班牙语)](file://messages/es/tools-image.json)
- [common.json (印尼语)](file://messages/id/common.json)
- [tools-audio.json (印尼语)](file://messages/id/tools-audio.json)
- [tools-developer.json (印尼语)](file://messages/id/tools-developer.json)
- [tools-image.json (印尼语)](file://messages/id/tools-image.json)
- [tools-pdf.json (印尼语)](file://messages/id/tools-pdf.json)
- [tools-video.json (印尼语)](file://messages/id/tools-video.json)
</cite>

## 更新摘要
**所做更改**
- 新增印尼语本地化重大扩展内容，涵盖6个JSON文件的完整翻译
- 更新了支持的语言列表，将印尼语正式纳入21种语言支持
- 增强了多语言用户体验优化策略，包含印尼语的完整工具翻译
- 更新了语言贡献指南，新增印尼语翻译质量保证标准
...
**更新** 印尼语本地化已实现重大扩展，涵盖所有工具类别的完整翻译，包括音频、开发者工具、图像处理、PDF和视频工具的详细功能描述和SEO内容。
...
EN["messages/en/*"]
ZH["messages/zh-Hans/*"]
ES["messages/es/*<br/>包含完整工具翻译"]
ID["messages/id/*<br/>印尼语完整翻译"]
OTHER["messages/{ar,de,fr,hi,it,ja,ko,nl,pl,pt-BR,pt-PT,ru,th,tr,uk,vi,zh-Hant}/*"]
end
R --> N
R --> REQ
REQ --> LM
...
**更新** 印尼语翻译现已覆盖所有工具类别，包括详细的SEO内容、FAQ和使用说明，提供完整的本地化体验。
...
**更新** 印尼语翻译现已包含完整的工具消息文件，涵盖音频、开发者工具、图像处理、PDF和视频工具的所有功能描述、SEO内容和FAQ。
...
**更新** 印尼语翻译的完整覆盖减少了客户端翻译缺失的情况，提高了整体性能表现。
...
- 消息缺失或乱码
  ...
  - 检查消息键名是否与组件引用一致
  - **更新** 对于印尼语，确认 tools-audio.json、tools-developer.json、tools-image.json、tools-pdf.json 和 tools-video.json 是否完整加载
...
**更新** 印尼语本地化的重大扩展显著提升了多语言用户体验，现在涵盖了所有工具类别的完整翻译，包括详细的SEO内容、FAQ和使用说明，为印尼语用户提供了一流的本地化体验。
...
**更新** 印尼语现已包含完整的工具翻译，涵盖音频、开发者工具、图像处理、PDF和视频等所有功能领域。
...
- 维护流程
  ...
  - **更新** 对于印尼语，确保 tools-audio.json、tools-developer.json、tools-image.json、tools-pdf.json 和 tools-video.json 全部完整翻译
  - 进行本地测试，确保消息键名与组件引用一致，RTL 方向正确

**更新** 印尼语维护流程现已包含完整的工具类别翻译验证步骤。
...
- [common.json (印尼语):1-509](file://messages/id/common.json#L1-L509)
...
- 文化适配
  ...
  - **更新** 印尼语作为非RTL语言，确保文本从左到右正确显示
- 无障碍与可访问性
  - 为语言切换器与横幅提供适当的 aria-label 与键盘导航
  - 确保切换后焦点管理与屏幕阅读器友好
- **更新** 印尼语SEO优化
  - 所有工具类别均包含详细的SEO内容，包括元标题、元描述和关键词
  - FAQ部分提供针对印尼语用户的本地化问答
  - 使用印尼语的自然表达方式和术语
...
- 贡献流程
  ...
  - **更新** 对于印尼语，确保包含以下文件的完整翻译：
    - messages/id/tools-audio.json（音频工具）
    - messages/id/tools-developer.json（开发者工具）
    - messages/id/tools-image.json（图像处理工具）
    - messages/id/tools-pdf.json（PDF工具）
    - messages/id/tools-video.json（视频工具）
...
- 质量保证
  ...
  - **更新** 印尼语质量保证：
    - 确保所有工具的SEO内容、FAQ和使用说明完整翻译
    - 验证印尼语语法和术语的准确性
    - 测试工具功能描述的本地化适配
  - 代码审查：PR 需经至少一名维护者审查通过

**更新** 印尼语贡献指南现已包含完整的工具类别翻译要求和质量保证标准。