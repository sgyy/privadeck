...
## 更新摘要
**变更内容**
- 新增PdfFullscreenPreview全屏PDF预览组件，替代原有的PdfBlobPreview组件
- 新增MergeItemCard PDF合并项目卡片组件，支持拖拽排序和多种文件类型
- 新增PdfDetailDialog PDF详情对话框组件，提供页面选择和旋转功能
- 更新@dnd-kit依赖版本，支持拖拽功能
- 所有PDF相关组件获得完整的键盘导航和可访问性支持
...
**更新** 本版本重点介绍了最新的PDF合并工具组件体系，包括PdfFullscreenPreview全屏预览、MergeItemCard拖拽卡片、PdfDetailDialog详情对话框等新增组件，以及更新的@dnd-kit依赖支持拖拽功能。所有组件现在具备完整的键盘导航和ARIA角色支持。
...
## 结论
共享组件通过统一的UI基元、工具函数与国际化体系，实现了高复用、低耦合与一致体验。**最新版本**显著提升了可访问性支持，包括完整的ARIA角色、键盘导航和无障碍标签，并新增了Google Analytics脚本集成、完善的分析埋点系统以及专业的TimeRangeSlider时间范围选择组件。**重大更新** 本版本新增了完整的PDF处理工具链，包括PdfFullscreenPreview全屏预览、MergeItemCard拖拽卡片、PdfDetailDialog详情对话框等组件，以及更新的@dnd-kit依赖支持拖拽功能。这些组件在文件处理、图片展示、文本输入、主题与语言切换、全局搜索、时间选择、PDF处理等方面提供了可配置、可扩展且性能友好的解决方案。建议在业务组件中优先组合使用这些共享组件，并遵循本文的接口规范、交互设计与性能优化建议。
