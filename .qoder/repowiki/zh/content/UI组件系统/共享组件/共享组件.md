...
## 更新摘要
**变更内容**
- 新增TimeRangeSlider时间范围滑块组件，用于视频处理工具的精确时间选择
- 新增Google Analytics脚本组件，支持GA4集成和环境变量配置
- LanguageSwitcher组件获得重大可访问性改进，包括ARIA角色、键盘导航支持
- 分析埋点系统完善，新增language_change事件类型和隐私保护机制
- Header组件增强键盘导航支持
- CompareSlider、FileDropzone、ImageLightbox等组件获得键盘操作支持
- 所有组件现在具备完整的键盘导航和ARIA角色支持
- 统一的可访问性设计模式和最佳实践
...
**更新** 本版本重点介绍了最新的可访问性改进、Google Analytics脚本集成和分析埋点系统的完善，包括ARIA角色、键盘导航支持、无障碍标签的全面实现以及隐私保护机制。新增了TimeRangeSlider时间范围滑块组件，专门用于视频处理工具的精确时间选择。

## 项目结构
共享组件集中位于 src/components/shared 目录，围绕"文件上传/下载"、"图片展示/预览/对比"、"文本输入/拖拽"、"主题/语言切换"、"全局搜索"、"分析埋点"、"时间范围选择"等通用能力构建，配合 src/components/ui/Button.tsx、src/components/ui/Dialog.tsx、src/lib/utils/* 工具函数与国际化消息 common.json 实现一致的外观与行为。
...
- TimeRangeSlider：时间范围滑块，双拇指拖拽、范围选择、最小持续时间约束，专为视频处理工具设计。
...
- 时间范围选择：TimeRangeSlider 为视频处理工具提供精确的时间选择能力。
...
- 功能特性
  - 双拇指拖拽选择时间范围，支持拖拽开始拇指、结束拇指和范围区域。
  - 自动约束最小持续时间，防止选择过短的片段。
  - 实时显示开始时间、持续时间和结束时间。
  - 专为视频处理工具设计，支持精确的时间选择。
- 属性接口
  - duration: number (总时长，秒)
  - startTime: number (开始时间，秒)
  - endTime: number (结束时间，秒)
  - minDuration?: number (最小持续时间，默认0.5秒)
  - onStartChange: (time: number) => void (开始时间变化回调)
  - onEndChange: (time: number) => void (结束时间变化回调)
- 事件与状态
  - 支持三种拖拽模式：start（开始拇指）、end（结束拇指）、range（范围区域）。
  - 拖拽时实时更新时间值，保持最小持续时间约束。
  - 自动边界检查，防止超出视频时长范围。
- 可配置性与扩展性
  - 可配置最小持续时间，适应不同视频处理需求。
  - 支持自定义时间格式化显示。
- 交互与可访问性
  - 基于Button组件，具备统一的焦点与无障碍标签。
  - 支持鼠标和触摸设备操作。
- 性能与内存
  - 使用useRef和useCallback优化性能，避免不必要的重渲染。
  - 仅在拖拽时计算时间值，其他时间保持不变。
- 使用示例
  - 视频剪辑、GIF制作、视频转码等工具中的时间选择功能。

**章节来源**
- [TimeRangeSlider.tsx:7-14](file://src/components/shared/TimeRangeSlider.tsx#L7-L14)
- [TimeRangeSlider.tsx:20-89](file://src/components/shared/TimeRangeSlider.tsx#L20-L89)
- [TimeRangeSlider.tsx:91-97](file://src/components/shared/TimeRangeSlider.tsx#L91-L97)
- [TimeRangeSlider.tsx:99-142](file://src/components/shared/TimeRangeSlider.tsx#L99-L142)
...
- **TimeRangeSlider**：基于Button组件的可访问性特性
...
- [TimeRangeSlider.tsx:127-139](file://src/components/shared/TimeRangeSlider.tsx#L127-L139)
...
- 组件耦合
  ...
  - **新增** TimeRangeSlider 依赖 Button 组件提供可访问性支持。
- 外部依赖
...
- DOM 与渲染
  ...
  - **新增** TimeRangeSlider 使用useCallback优化拖拽性能。
- 资源与体积
...
- [TimeRangeSlider.tsx:59-84](file://src/components/shared/TimeRangeSlider.tsx#L59-L84)
...
- **新增** TimeRangeSlider时间选择问题
  - 检查duration、startTime、endTime参数是否正确传递。
  - 确认minDuration设置是否合理。
  - 验证onStartChange和onEndChange回调是否正常触发。
  - 检查拖拽边界约束是否按预期工作。
...
## 结论
共享组件通过统一的UI基元、工具函数与国际化体系，实现了高复用、低耦合与一致体验。**最新版本**显著提升了可访问性支持，包括完整的ARIA角色、键盘导航和无障碍标签，并新增了Google Analytics脚本集成和完善的分析埋点系统。新增的TimeRangeSlider组件专门用于视频处理工具的精确时间选择，支持双拇指拖拽、范围选择和最小持续时间约束。它们在文件处理、图片展示、文本输入、主题与语言切换、全局搜索、时间选择等方面提供了可配置、可扩展且性能友好的解决方案。建议在业务组件中优先组合使用这些共享组件，并遵循本文的接口规范、交互设计与性能优化建议。
...
- **新增** TimeRangeSlider使用清单
  - 正确设置duration、startTime、endTime参数
  - 合理配置minDuration以满足视频处理需求
  - 实现onStartChange和onEndChange回调处理时间变化
  - 注意拖拽边界约束和最小持续时间限制