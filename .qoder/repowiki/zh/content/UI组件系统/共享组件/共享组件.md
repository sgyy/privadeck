...
## 更新摘要
**变更内容**
- 新增ToolAnalyticsContext工具分析上下文组件，提供工具级别的分析数据传递
- 新增ToolCardLink工具卡片链接组件，支持工具卡片点击分析埋点
- 增强分析埋点系统，新增tool_card_click事件类型和trackToolCardClick函数
- 更新工具页面架构，通过ToolPageShell组件提供工具分析上下文
- 扩展工具导航组件，支持位置信息传递和分析埋点
...
**更新** 本版本重点介绍了新增的ToolAnalyticsContext和ToolCardLink组件，以及增强的分析埋点系统。这些新组件为工具页面提供了统一的分析上下文管理和工具卡片点击追踪功能，完善了整个分析埋点体系。

## 项目结构
共享组件集中位于 src/components/shared 目录，围绕"文件上传/下载"、"图片展示/预览/对比"、"文本输入/拖拽"、"主题/语言切换"、"全局搜索"、"分析埋点"、"时间范围选择"、"PDF处理"等通用能力构建，配合 src/components/ui/Button.tsx、src/components/ui/Dialog.tsx、src/lib/utils/* 工具函数与国际化消息 common.json 实现一致的外观与行为。

新增的工具分析组件位于 src/components/tool 目录，包括 ToolAnalyticsContext、ToolCardLink、ToolPageShell 等组件，为工具页面提供分析上下文和工具卡片链接功能。
...
**新增** 本版本最重要的更新是引入了ToolAnalyticsContext、ToolCardLink、ToolPageShell和RelatedTools等新的分析组件，为工具页面提供了统一的分析上下文管理和工具卡片点击追踪功能。这些组件通过类型安全的API和完整的分析埋点系统，为工具页面的用户行为分析提供了强大的支持。

这些组件在文件处理、图片展示、文本输入、主题与语言切换、全局搜索、时间选择、PDF处理、工具分析等方面提供了可配置、可扩展且性能友好的解决方案。建议在业务组件中优先组合使用这些共享组件，并遵循本文的接口规范、交互设计与性能优化建议。
...
- 可访问性清单
  ...
  - **新增** ToolAnalyticsContext作为上下文组件，不直接参与用户交互。
