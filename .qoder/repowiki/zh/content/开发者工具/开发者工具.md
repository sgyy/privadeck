...
- [logic.ts（JSON 编辑器）](file://src/tools/developer/json-editor/logic.ts)
...
## 简介
本文件面向开发者工具模块，系统性梳理并解释 18 个开发者辅助工具：Base64 编解码、哈希生成、JSON 格式化与校验、**JSON 编辑器**、URL 编解码、CSV/TSV 与 JSON 互转、时间戳与日期互转、颜色格式转换、正则表达式测试、Markdown 预览、文本差异对比、大小写与命名风格转换、YAML/JSON 转换、JSON/XML 转换、OCR 文字识别、字数统计、归档打包、Lorem Ipsum 模拟文本。文档覆盖技术实现原理、数据格式与编码标准、处理算法、使用方法与参数配置、批量与格式转换选项、性能特点与最佳实践，并提供可直接定位到源码的路径以便进一步查阅。
...
D["JSON 编辑器<br/>JsonEditor.tsx"]
E["URL 编码器<br/>UrlEncoder.tsx"]
F["CSV/JSON 转换<br/>CsvJsonTool.tsx"]
G["时间戳转换<br/>Timestamp.tsx"]
H["颜色转换<br/>ColorConverter.tsx"]
I["正则测试器<br/>RegexTester.tsx"]
J["Markdown 预览<br/>MarkdownPreview.tsx"]
K["文本差异<br/>TextDiff.tsx"]
L["大小写转换<br/>CaseConverter.tsx"]
M["YAML/JSON 转换<br/>YamlJson.tsx"]
N["JSON/XML 转换<br/>JsonXmlTool.tsx"]
O["OCR 文字识别<br/>Ocr.tsx"]
P["字数统计<br/>WordCounter.tsx"]
Q["归档打包<br/>Archive.tsx"]
R["Lorem Ipsum<br/>LoremIpsum.tsx"]
end
A --> A1["logic.tsBase64"]
B --> B1["logic.ts哈希"]
C --> C1["logic.tsJSON 格式化"]
D --> D1["logic.tsJSON 编辑器"]
E --> E1["logic.tsURL 编码"]
F --> F1["logic.tsCSV/JSON"]
G --> G1["logic.ts时间戳"]
H --> H1["logic.ts颜色"]
I --> I1["logic.ts正则"]
J --> J1["logic.tsMarkdown"]
K --> K1["logic.ts差异"]
L --> L1["logic.ts大小写"]
M --> M1["logic.tsYAML/JSON"]
N --> N1["logic.tsJSON/XML"]
O --> O1["logic.tsOCR"]
P --> P1["logic.ts字数统计"]
Q --> Q1["logic.ts归档"]
R --> R1["logic.tsLorem Ipsum"]
```
...
## 核心组件
- Base64 工具：提供字符串与二进制之间的 Base64 编解码，支持拖拽文本输入与结果复制下载。
- 哈希生成器：支持文本与文件两种输入模式，计算多种哈希值（如 SHA-256、MD5 等），并可一键复制结果。
- JSON 格式化：支持格式化、压缩与校验，可自定义缩进空格数，实时显示校验结果。
- **JSON 编辑器**：提供交互式 JSON 编辑功能，支持三种视图模式（文本、树形、预览），内置撤销/重做历史，自动修复功能，支持节点增删改查和类型转换。
- URL 编码器：支持 URL 整体与组件级编解码，适用于查询参数、路径等场景。
- CSV/JSON 转换：双向转换 CSV/TSV 与 JSON，支持拖拽文件与错误提示。
...
- YAML/JSON 转换：双向转换 YAML 与 JSON，支持拖拽与复制下载。
- JSON/XML 转换：双向转换 JSON 与 XML，支持拖拽与错误提示。
- OCR 文字识别：支持多语言图片上传识别，显示置信度与进度。
- 字数统计：统计词数、字符数、句数、段落数与阅读时长。
- 归档打包：将多个文件打包为压缩包（具体实现见 logic.ts）。
...
## 架构总览
各工具页面组件通过 React Hooks 管理状态，调用对应 logic.ts 中的纯函数进行数据处理，再将结果反馈到 UI。部分工具涉及浏览器原生 API 或外部库（如 OCR、Monaco 编辑器），并在 UI 层提供进度与错误提示。

```mermaid
graph TB
UI["页面组件React 客户端"] --> LOGIC["逻辑模块logic.ts"]
LOGIC --> NATIVE["浏览器 API/原生能力"]
LOGIC --> UTIL["通用工具函数"]
UI --> FEAT["功能特性复制/下载/拖拽"]
FEAT --> UI
```
...
### Base64 工具
- 功能：对任意文本进行 Base64 编码与解码；支持从文件拖拽读取文本。
- 技术要点：使用浏览器原生编码/解码能力；输出结果支持复制与下载为文本文件。
- 使用方法：在输入框粘贴或拖拽文本，点击"编码"或"解码"，查看输出区域并可复制/下载。
- 参数配置：无额外参数；输出为纯文本。
- 性能特点：纯前端处理，延迟极低；大文本时注意内存占用。
- 最佳实践：大文件建议先转为文本再处理；注意 Base64 增大约 33% 的体积。
...
### JSON 格式化
- 功能：格式化、压缩与校验 JSON；可自定义缩进空格数。
- 技术要点：格式化与压缩分别调用不同策略；校验返回布尔值与错误信息。
- 使用方法：粘贴或拖拽 JSON，选择"格式化/压缩/校验"，设置缩进空格数，查看结果。
- 参数配置：缩进空格数（2/4/8）。
- 性能特点：超大 JSON 可能导致 UI 卡顿；建议分段处理或限制输入长度。
- 最佳实践：生产环境优先使用压缩版本；格式化仅用于调试。
...
### JSON 编辑器
- 功能：提供交互式 JSON 编辑功能，支持三种视图模式（文本、树形、预览），内置撤销/重做历史，自动修复功能，支持节点增删改查和类型转换。
- 技术要点：使用 Monaco 编辑器提供语法高亮和智能补全；树形视图支持双击编辑、右键菜单、节点拖拽；历史管理支持 50 步撤销/重做；自动修复使用 jsonrepair 库。
- 使用方法：在三种视图间切换，使用树形视图进行可视化编辑，或使用文本视图进行传统编辑；支持键盘快捷键（Ctrl+Z/Ctrl+Y）。
- 参数配置：视图模式切换、撤销/重做步数、自动修复阈值。
- 性能特点：树形视图对大型 JSON 结构可能有性能影响；建议分段处理或使用预览模式。
- 最佳实践：大型 JSON 建议使用智能格式化；频繁编辑时启用撤销/重做；自动修复功能可快速恢复损坏的 JSON。

**章节来源**
- [JsonEditor.tsx:1-425](file://src/tools/developer/json-editor/JsonEditor.tsx#L1-L425)
- [logic.ts（JSON 编辑器）](file://src/tools/developer/json-editor/logic.ts)
- [EditableJsonTree.tsx:1-771](file://src/tools/developer/json-editor/EditableJsonTree.tsx#L1-L771)
...
- 技术要点：解析逗号分隔与 JSON 数组；异常时抛出可读错误信息。
...
### 时间戳转换
- 功能：秒级与毫秒级时间戳与日期互转；自动识别单位；输出 UTC、本地、ISO 与相对时间。
...
- 外部依赖：Base64/URL 编解码使用浏览器原生 API；OCR 使用外部模型（逻辑层封装）；Markdown 预览使用 HTML 渲染；**JSON 编辑器使用 Monaco 编辑器和 jsonrepair 库**。
...
- **JSON 编辑器性能**：树形视图对大型 JSON 结构可能有性能影响；Monaco 编辑器占用较多内存；建议使用智能格式化和预览模式。
...
- **JSON 编辑器问题**：
  - 视图切换失败：检查 JSON 语法有效性；使用自动修复功能。
  - 树形视图无响应：检查 JSON 结构复杂度；考虑使用文本视图。
  - 撤销/重做失效：检查历史记录是否被清空；确认最大历史步数限制。
...
## 结论
该开发者工具模块以清晰的页面组件与纯函数逻辑分离为核心架构，覆盖了开发工作中的高频场景。**新增的 JSON 编辑器工具显著增强了 JSON 数据的编辑体验，提供了三种视图模式、撤销/重做历史、自动修复等功能，满足了复杂的 JSON 结构编辑需求。**通过标准化的输入输出与错误处理机制，工具具备良好的可用性与可维护性。建议在实际使用中结合性能优化与最佳实践，以获得更流畅的开发体验。

## 附录
- 使用示例与案例：可在各工具页面中直接输入样例数据进行演示，例如 JSON 格式化可粘贴一段缩进不规范的 JSON 进行格式化；**JSON 编辑器可使用复杂嵌套结构进行可视化编辑**；正则测试器可使用常见邮箱/手机号正则进行验证；OCR 可上传清晰图片进行识别。
- 批量处理与格式转换：归档工具支持多文件打包；大小写转换支持批量文本；CSV/JSON/YAML/JSON/XML 支持拖拽批量导入。
- 参数配置清单：缩进空格数（JSON 格式化）、标志位组合（正则测试）、语言选择（OCR）、命名风格（大小写转换）、**视图模式（JSON 编辑器）**、撤销/重做步数（JSON 编辑器）等。