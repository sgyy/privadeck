...
媒体工具箱采用基于 next-intl 的现代化国际化系统，支持 22 种语言和地区变体，实现了完整的多语言网站功能。该系统通过语言前缀路由、智能语言检测、动态翻译加载和 SEO 友好的元数据管理，为全球用户提供本地化的媒体处理体验。
...
国际化系统的核心是基于 next-intl 的路由配置，定义了支持的语言列表和默认行为：
...
系统支持 22 种语言，其中阿拉伯语（ar）被标记为 RTL 语言，确保正确的文本方向。
...
翻译文件采用模块化组织，分为通用消息和工具特定消息：

```mermaid
graph LR
subgraph "消息结构"
A[common.json] --> A1[通用界面文本]
A --> A2[导航菜单]
A --> A3[页脚信息]
A --> A4[隐私政策]
A --> A5[常见问题]
B[tools-*.json] --> B1[开发者工具]
B --> B2[图像工具]
B --> B3[PDF工具]
B --> B4[视频工具]
B --> B5[音频工具]
...
- [tools-video.json:1-814](file://messages/en/tools-video.json#L1-L814)
...
User->>Router : 访问 /zh-Hans/tools/image/compress/
Router->>Layout : 加载 [locale] 布局
Layout->>Messages : 加载通用翻译
Layout->>Messages : 加载工具分类翻译
...
媒体工具箱的国际化系统通过精心设计的架构和实现，成功地为全球用户提供了无缝的多语言体验。系统的主要优势包括：

1. **完整的语言支持**：支持 22 种语言和地区变体
