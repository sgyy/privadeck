...
## 更新摘要
**变更内容**
- **重大更新**：从阿拉伯语翻译文件中移除了约800+行视频工具相关翻译，包括mute、trim、rotate、video-to-GIF、video-to-WebP、compress等工具的完整翻译
- **影响范围**：移除了阿拉伯语视频工具翻译文件中的完整WebP功能翻译，强调WebP优势和现代WebP功能
...
- 更新语言支持列表，现支持22种语言和地区变体（**减少了2个语言支持**）
- **移除阿拉伯语视频工具翻译（约820行）** - 本次重大更新的核心变更
- **移除乌克兰语视频工具翻译（约800行）** - 本次重大更新的重要变更
...
**更新** 新增乌克兰语完整本地化支持，现包含通用界面、图像工具、PDF工具的完整翻译内容。乌克兰语翻译现已覆盖所有主要功能区域，总计约509KB的数据量，包括：

- 通用界面元素：509行翻译，包含站点名称、描述、工具栏、导航等
- 图像工具：800行翻译，包含图像转换、压缩、裁剪、滤镜等功能
- PDF工具：681行翻译，包含PDF编辑、转换、压缩等功能
- **视频工具：已移除（约800行）** - 本次更新的重要变更

**更新** 新增深度合并功能和英语回退机制，显著提升了翻译系统的可靠性和性能。系统现在能够智能地合并翻译文件，确保即使部分翻译缺失也能正常工作。

**更新** 新增越南语和阿拉伯语完整本地化支持，现包含通用界面、音频工具、开发者工具、图像工具、PDF工具的完整翻译内容。阿拉伯语翻译现已覆盖所有主要功能区域，总计约509KB的数据量，包括：
...
- PDF工具：681行翻译，包含PDF编辑、转换、压缩等功能
- **视频工具：已移除（约820行）** - 本次更新的重要变更，包含WebP功能的完整翻译
...
系统现在支持22种语言，其中乌克兰语（uk）被标记为 RTL 语言，确保正确的文本方向。
...
**更新** 翻译文件采用模块化组织，分为通用消息和工具特定消息。越南语、阿拉伯语和乌克兰语现已完全支持（**视频工具翻译已移除**）：
...
**更新** 阿拉伯语翻译现已移除视频工具相关翻译，仅包含通用界面、音频工具、开发者工具、图像工具、PDF工具的翻译内容：
- 通用界面元素：509行翻译，包含站点名称、描述、工具栏、导航等
- 音频工具：191行翻译，覆盖音频剪辑、转换、提取等功能
- 开发者工具：950行翻译，覆盖所有开发工具的名称、描述、SEO内容
- 图像工具：921行翻译，包含图像转换、压缩、裁剪、滤镜等功能
- PDF工具：681行翻译，包含PDF编辑、转换、压缩等功能
- **视频工具：已移除（约820行）** - 本次更新的重要变更

乌克兰语翻译现已移除视频工具相关翻译，仅包含通用界面、图像工具、PDF工具的翻译内容：
- 通用界面元素：509行翻译，包含站点名称、描述、工具栏、导航等
- 图像工具：800行翻译，包含图像转换、压缩、裁剪、滤镜等功能
- PDF工具：681行翻译，包含PDF编辑、转换、压缩等功能
- **视频工具：已移除（约800行）** - 本次更新的重要变更
...
User->>Router : 访问 /ar/tools/image/compress/
...
**更新** 工具页面实现了按需翻译加载，确保性能和用户体验（**视频工具页面翻译已移除**）：
...
- **视频工具页面：已移除（约820行）** - 本次更新的重要变更

乌克兰语工具页面的完整支持包括：
- 图像工具页面：800行翻译，覆盖图像转换、压缩、裁剪、滤镜等功能
- PDF工具页面：681行翻译，覆盖合并、拆分、压缩、转换等PDF功能
- **视频工具页面：已移除（约800行）** - 本次更新的重要变更
...
- **阿拉伯语**：common.json（509行）、tools-audio.json（191行）、tools-developer.json（950行）、tools-image.json（921行）、tools-pdf.json（681行）、**tools-video.json（已移除）**
- **乌克兰语**：common.json（509行）、tools-image.json（800行）、tools-pdf.json（681行）、**tools-video.json（已移除）**
...
媒体工具箱的国际化系统通过精心设计的架构和实现，成功地为全球用户提供了无缝的多语言体验。系统现已支持22种语言，包括最新的越南语完整本地化支持，以及阿拉伯语和乌克兰语翻译的移除，体现了项目国际化策略的调整。
...
越南语、阿拉伯语和乌克兰语的完整加入进一步增强了系统的国际化能力，为这三个重要市场的用户提供了本地化的媒体处理体验。虽然移除了部分视频工具翻译，但系统仍然为这些语言提供了完整的通用界面、音频工具、开发者工具、图像工具和PDF工具的翻译支持。
