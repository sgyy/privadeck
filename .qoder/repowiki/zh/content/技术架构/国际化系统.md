...
- PDF工具页面：681行翻译，覆盖合并、拆分、压缩、转换等PDF功能
- 视频工具页面：800行翻译，覆盖格式转换、压缩、编辑等视频处理功能
...
- **乌克兰语**：common.json（509行）、tools-image.json（800行）、tools-pdf.json（681行）、tools-video.json（800行），总计约509KB
...
媒体工具箱的国际化系统通过精心设计的架构和实现，成功地为全球用户提供了无缝的多语言体验。系统现已支持24种语言，包括最新的越南语、阿拉伯语和乌克兰语完整本地化支持，以及深度合并功能、英语回退机制和缓存优化等创新功能。
