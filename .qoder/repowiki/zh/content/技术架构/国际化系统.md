...
**更新** 新增越南语完整本地化支持，现包含通用界面、音频工具、开发者工具、图像工具、PDF工具和视频工具的完整翻译内容。越南语翻译现已覆盖所有主要功能区域，总计约83KB的翻译数据，包括：
...
G --> G2[logic.ts]
H[src/i18n/request.ts] --> H1[timeZone: "UTC"]
end
```
...
E --> G[languageNames.ts]
H[LocaleSuggestionBanner.tsx] --> A
H --> I[detectLocale.ts]
J[ToolPageShell.tsx] --> K[common.json]
J --> L[tools-*.json]
M[Timestamp.tsx] --> F
end
subgraph "应用层依赖"
...
P[page.tsx] --> A
P --> O
Q[ToolPageClient.tsx] --> R[registry]
S[logic.ts] --> S1[Date API]
end
subgraph "工具函数依赖"
O --> T[toolNavData.ts]
...
该系统为类似项目的国际化实现提供了优秀的参考模板，展示了如何在保持性能的同时提供丰富的本地化功能。
