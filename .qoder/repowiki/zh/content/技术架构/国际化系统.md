...
## 更新摘要
**变更内容**
- **重大更新**：国际化系统移除了安装提示相关的翻译键，包括 installPrompt、install、dismiss 翻译键已从所有语言文件的 common.json 中删除
- **影响范围**：涉及阿拉伯语、德语、英语、西班牙语、法语、意大利语、荷兰语、简体中文、繁体中文等语言环境
- **更新内容**：更新了相关章节以反映翻译键的移除，并提供了相应的故障排除指南
...
**更新** 安装提示翻译键移除的重要意义：
- **简化国际化配置**：移除了不再使用的翻译键，减少了维护复杂性
- **提升系统性能**：减少了翻译文件的大小和加载时间
- **统一用户体验**：消除了不同语言环境下安装提示的不一致性
- **降低维护成本**：减少了翻译维护工作量和潜在的翻译错误
...
**更新** 翻译文件采用模块化组织，分为通用消息和工具特定消息。所有33种语言都包含完整的JSON编辑器翻译内容：
...
User->>Router : 访问 /ar/tools/developer/json-editor/
...
**更新** 工具页面实现了按需翻译加载，确保性能和用户体验。所有33种语言都包含完整的JSON编辑器翻译：
...
W[page.tsx] --> A
W --> G
X[ToolPageClient.tsx] --> Y[registry]
Z[logic.ts] --> AA[Date API]
end
subgraph "工具函数依赖"
G --> BB[toolNavData.ts]
CC[loadMessages.ts] --> DD[messages/*]
DD --> DE[messages/ar/]
DE --> DF[ar/common.json]
DE --> DG[ar/tools-*.json]
DD --> DH[messages/vi/]
DH --> DI[vi/common.json]
DH --> DJ[vi/tools-*.json]
DD --> DK[messages/uk/]
DK --> DL[uk/common.json]
DK --> DM[uk/tools-*.json]
DN[metadata.ts] --> A
DO[sitemap.ts] --> A
DP[JsonEditor.tsx] --> DD
end
```
...
媒体工具箱的国际化系统通过精心设计的架构和实现，成功地为全球用户提供了无缝的多语言体验。系统现已支持33种语言，包括新增的JSON编辑器多语言支持，实现了完整的多语言媒体处理体验。
...
13. **安装提示翻译键移除**：简化了国际化配置，提升了系统性能

**更新** 新增JSON编辑器多语言支持的重大意义：
- **全面覆盖**：从原有的约22种语言扩展到33种语言的完整支持
- **翻译完整性**：每个语言环境都包含JSON编辑器的名称、描述、功能卡片、FAQ、SEO内容和界面文本
- **工具页面集成**：JSON编辑器工具页面现在支持完整的多语言本地化
- **用户体验提升**：用户可以在任何支持的语言环境中使用JSON编辑器功能
