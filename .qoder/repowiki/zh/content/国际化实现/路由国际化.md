...
- [tools-pdf.json](file://messages/id/tools-pdf.json)
...
PrivaDeck 媒体工具箱采用基于语言前缀的路由国际化系统，通过 Next.js 的 next-intl 库实现多语言支持。该系统支持 21 种语言和地区变体，包括英语、简体中文、繁体中文、日语、韩语等主要语言，以及阿拉伯语、俄语、土耳其语、印尼语等其他语言。

系统的核心设计是将语言代码作为 URL 的第一段路径，如 `/en`、`/zh-Hans`、`/zh-Hant`、`/id` 等，形成清晰的语言前缀路由结构。这种设计确保了搜索引擎友好性和用户可预测性。

**更新** 印尼语（id）已正式纳入语言支持列表，成为第21种支持的语言，进一步扩大了系统的国际化覆盖范围。
...
**更新** 印尼语（id）已成功集成到语言配置中，作为第21种支持的语言，扩展了系统的东南亚语言覆盖能力。
...
User->>Router : 访问 /id/tools/audio/convert/
...
1. **URL 语言参数**: 从 URL 路径中提取语言代码，包括新增的印尼语支持
...
1. **静态参数生成**: 为每个支持的语言生成静态路由参数，包括印尼语
...
**更新** 印尼语本地化的成功集成进一步证明了系统的强大扩展能力，为东南亚地区的用户提供了更好的本地化体验。这一扩展不仅增加了语言支持数量，更重要的是体现了系统对新兴市场的关注和适应能力。
