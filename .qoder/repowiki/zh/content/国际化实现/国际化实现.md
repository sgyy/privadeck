...
- [tools-video.json](file://messages/fr/tools-video.json)
- [common.json](file://messages/hi/common.json)
- [tools-audio.json](file://messages/hi/tools-audio.json)
- [common.json](file://messages/id/common.json)
- [tools-image.json](file://messages/id/tools-image.json)
- [tools-video.json](file://messages/id/tools-video.json)
- [tools-audio.json](file://messages/id/tools-audio.json)
- [tools-developer.json](file://messages/id/tools-developer.json)
</cite>

## 更新摘要
**所做更改**
- 新增印尼语（id）本地化支持，完善多语言覆盖范围
- 完善翻译资源配置，增强SEO优化和文化适配能力
- 更新语言支持列表，包含新增的印尼语支持
- 增强RTL语言支持的实现细节
- 补充语言检测机制的多层策略
- 优化翻译资源管理的性能考虑
...
PrivaDeck 媒体工具箱采用 next-intl 框架实现了完整的国际化系统，现已支持 22 种语言和地区变体，包括新增的印尼语本地化支持。该系统通过动态语言切换、路由国际化、RTL 语言支持和智能语言建议等功能，为全球用户提供本地化的媒体处理体验。

**更新** 完善了国际化架构，特别注重SEO优化和文化适配，确保多语言内容在搜索引擎和用户体验方面的最佳表现。新增的印尼语翻译涵盖了通用界面、图像处理工具、视频处理工具、音频处理工具和开发者工具的完整功能集，进一步增强了系统的全球化服务能力。
...
- **阿拉伯语**: ar（阿拉伯语，支持 RTL）
- **新增印尼语**: id（Bahasa Indonesia）
- **新增欧洲语言**: de（德语）、fr（法语）、it（意大利语）、nl（荷兰语）、pl（波兰语）、ru（俄语）、tr（土耳其语）、uk（乌克兰语）
- **亚洲语言**: ja（日语）、ko（韩语）、th（泰语）、vi（越南语）、hi（印地语）
- **其他语言**: en（英语）、es（西班牙语）
...
```mermaid
graph TD
A[messages/] --> B[语言目录<br/>如: en/, zh-Hans/, ar/, de/, fr/, hi/, id/]
...
**图表来源**
- [common.json:1-509](file://messages/id/common.json#L1-L509)
...
Browser->>Router : 访问 /id/tools/image/compress/
Router->>Config : 解析语言代码 "id"
...
1. **语言前缀路由**: 使用 `/de/`、`/fr/`、`/id/` 等语言前缀明确标识语言
...
PrivaDeck 的国际化系统通过精心设计的架构和实现，为用户提供了一个完整、高效、可扩展的多语言支持解决方案。系统现已支持 22 种语言和地区变体，包括新增的印尼语本地化支持，主要优势包括：

1. **全面的语言支持**: 支持 22 种语言和地区变体，包括印尼语在内的新增语言
2. **灵活的路由设计**: 通过语言前缀实现真正的多语言路由
3. **智能的用户体验**: 提供语言建议和无缝切换
4. **优秀的性能表现**: 通过按需加载和预渲染优化性能
5. **完善的 RTL 支持**: 为阿拉伯语等语言提供完整的文本方向处理
6. **完整的本地化覆盖**: 新增语言翻译涵盖通用界面、图像处理工具、视频处理工具、音频处理工具和开发者工具的所有功能
7. **SEO优化**: 通过多层语言检测和路由优化搜索引擎可见性
8. **文化适配**: 确保内容在不同文化背景下的适当性和相关性

**更新** 本次更新重点完善了国际化架构，特别加强了印尼语本地化支持和SEO优化能力，为PrivaDeck在全球市场的推广奠定了坚实的技术基础。通过智能的语言检测、预渲染优化和多层缓存策略，系统能够为全球用户提供最佳的本地化体验，同时确保搜索引擎能够正确识别和索引多语言内容。
