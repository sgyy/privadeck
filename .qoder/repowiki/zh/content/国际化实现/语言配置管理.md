...
- [tools-pdf.json](file://messages/de/tools-pdf.json)
- [tools-video.json](file://messages/de/tools-video.json)
</cite>

## 更新摘要
**所做更改**
- 更新了语言配置数组，将德语（de）添加到支持的语言列表中
- 更新了语言名称映射，添加了德语的显示名称
- 新增了完整的德语语言资源文件（common.json、tools-audio.json、tools-image.json、tools-developer.json、tools-pdf.json、tools-video.json）
- 更新了语言配置对路由系统影响的说明，反映了新增德语的支持
...
PrivaDeck 媒体工具箱采用基于 Next.js International (next-intl) 的国际化系统，现已支持 19 种语言和地区变体。该系统提供了完整的语言配置管理功能，包括语言检测、类型安全、RTL 语言支持以及动态路由集成。

**更新** 新增德语（German）支持，扩展了平台的多语言能力，为德国用户提供本地化的用户体验。
...
**更新** 德语（"de"）已添加到语言配置数组中，位于西班牙语（"es"）和葡萄牙语（"pt-BR"）之间，体现了德语作为重要欧洲语言的地位。
...
**更新** 对于德语（"de"）的添加，已在现有配置中完成，无需额外修改。
...
**更新** 德语资源文件已完整创建，包含所有必要的语言文件：
- `messages/de/common.json` - 509 行，包含完整的通用消息
- `messages/de/tools-audio.json` - 191 行，包含音频工具消息
- `messages/de/tools-image.json` - 822 行，包含图像工具消息
- `messages/de/tools-developer.json` - 809 行，包含开发者工具消息
- `messages/de/tools-pdf.json` - 681 行，包含 PDF 工具消息
- `messages/de/tools-video.json` - 544 行，包含视频工具消息
...
**更新** 新增德语支持进一步增强了系统的国际化能力，为德国用户提供了本地化的用户体验。德语资源的完整实现包括通用消息、音频工具、图像工具、开发者工具、PDF 工具和视频工具的所有翻译，确保用户获得一致的本地化体验。
