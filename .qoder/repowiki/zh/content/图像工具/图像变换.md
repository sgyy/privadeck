...
## 更新摘要
**变更内容**
- 工具名称从 "Flip" 重命名为 "Flip & Rotate"
- 新增四种变换模式：水平翻转、垂直翻转、顺时针90°、逆时针90°
- 实现操作堆叠功能，支持多步变换组合
- 更新UI界面，提供直观的变换控制面板
- 增强变换预览和实时反馈机制
...
## 项目结构
图像变换相关的核心位置如下：
- 图像翻转（现已重命名为 Flip & Rotate）：逻辑层位于 src/tools/image/flip/logic.ts，前端组件位于 src/tools/image/flip/Flip.tsx，工具注册位于 src/tools/image/flip/index.ts
...
### 组合使用与创意应用
- 组合示例
  - 先翻转（水平/垂直），再旋转（90°顺时针/逆时针），最后按需压缩
  - 先旋转（90°），再翻转（水平/垂直），最后导出
- 创意应用
  - 利用翻转和旋转实现对称构图与镜像艺术
  - 结合多种变换实现复杂的图像处理效果
