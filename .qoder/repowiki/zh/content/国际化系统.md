...
**更新** 本次更新重点关注国际化的大幅扩展，支持22种语言，包括乌克兰语加入和阿拉伯语RTL布局支持。系统现已支持英语、简体中文、繁体中文、日语、韩语、西班牙语、法语、德语、巴西葡萄牙语、葡萄牙语、泰语、越南语、印尼语、印地语、阿拉伯语、意大利语、荷兰语、波兰语、俄语、土耳其语、乌克兰语等语言，显著扩大了全球用户覆盖范围。
...
graph TB
subgraph "国际化配置"
RT["routing.ts<br/>定义22种语言支持与默认语言"]
...
NAMES["languageNames.ts<br/>22种语言名称映射"]
end
subgraph "翻译资源"
COMMON["common.json<br/>通用文本与导航等"]
TOOLS["tools-*.json<br/>各工具分类文本"]
UK["乌克兰语翻译<br/>完整覆盖各工具模块"]
AR["阿拉伯语翻译<br/>RTL布局支持"]
EN["英语基础翻译"]
KO["韩语翻译<br/>开发者工具、音频、图像、视频完整覆盖"]
END
RT --> NAV
RT --> REQ
RT --> LAYOUT
...
## 核心组件
- 语言路由与默认语言：在路由配置中声明支持的22种语言列表、默认语言与 RTL 语言集合，用于后续导航与页面生成。
...
Layout->>Routing : 校验 locale 是否在22种支持语言列表
alt 有效
...
**图表来源**
- [layout.tsx:18-64](file://src/app/[locale]/layout.tsx#L18-L64)
- [routing.ts:1-18](file://src/i18n/routing.ts#L1-L18)
- [request.ts:1-20](file://src/i18n/request.ts#L1-L20)
- [loadMessages.ts:1-56](file://src/lib/i18n/loadMessages.ts#L1-L56)
...
### 语言路由与导航
- 语言列表与默认语言：在路由配置中定义支持的22种语言数组、默认语言与 RTL 语言数组，供导航与布局使用。
...
GetLocale --> Validate{"是否在22种支持语言列表?"}
...
### 应用布局与 RTL 支持
- 语言参数校验：生成静态参数时包含所有22种支持语言，运行时校验 locale 合法性，非法则 404。
...
### 翻译系统扩展与改进
**更新** 本次更新重点加强了乌克兰语和阿拉伯语翻译系统的完整性：

#### 乌克兰语翻译系统全面改进
- **开发者工具模块**：完整覆盖 JSON 格式化器、Base64 编码器、URL 编码器、OCR 文字识别、ZIP 解压、CSV/JSON 转换、哈希生成器、颜色转换器、JSON/XML 转换、Markdown 预览、正则表达式测试器、时间戳转换、YAML/JSON 转换、文本比较、单词计数器、大小写转换、Lorem Ipsum 生成器等 16 个工具的完整乌克兰语翻译。
- **音频工具模块**：完整覆盖音频裁剪、音频转换、音频提取、音量调节等 4 个工具的详细乌克兰语翻译，包括 SEO 内容和 FAQ 部分。
...
- **视频工具模块**：完整覆盖视频静音、视频裁剪、视频旋转、GIF 转换、WebP 转换、视频压缩、格式转换、视频尺寸调整、视频信息查看等 9 个工具的详细乌克兰语翻译，包括高级设置和功能卡片内容。
...
- **RTL布局适配**：系统已支持阿拉伯语的从右到左文本方向，确保界面元素正确镜像显示。

这些改进显著提升了多语言用户的使用体验，特别是在专业开发工具和多媒体编辑工具方面的本地化程度。

**章节来源**
- [common.json:1-508](file://messages/uk/common.json#L1-L508)
- [tools-image.json:1-800](file://messages/uk/tools-image.json#L1-L800)
- [tools-pdf.json:1-714](file://messages/uk/tools-pdf.json#L1-L714)
...
**更新** 本次乌克兰语和阿拉伯语翻译系统的全面改进显著增强了多语言用户的使用体验，特别是开发者工具、音频工具、图像工具、视频工具等专业领域的本地化程度大幅提升，为全球用户提供了更加完善的服务。
...
这些改进显著提升了多语言用户的整体使用体验，特别是乌克兰语和阿拉伯语用户的本地化程度达到了专业级水平。
